...
\section{Modificări realizate în fișierul \texttt{top.v}}
Pentru funcționarea corectă a implementării am realizat următoarele modificări în scheletului inițial:
\begin{itemize}
    \item Eliminarea redeclarărilor de tip \texttt{wire} pentru semnalele \texttt{clk}, \texttt{rst\_n}, \texttt{sclk}, \texttt{cs\_n}, \texttt{miso} și \texttt{mosi}, deoarece acestea sunt deja declarate ca porturi de intrare/ieșire în antetul modulului.
    \item La instanțierea modulului \texttt{spi\_bridge}, am inversat conectarea semnalelor \texttt{miso} și \texttt{mosi} pentru a respecta direcția datelor (input la input, output la output) și am adăugat parametrii lipsă: \texttt{byte\_sync}, \texttt{data\_in}, \texttt{data\_out}.
    \item La instanțierea modulului \texttt{instr\_dcd}, am adăugat parametrul \texttt{byte\_sync}.
\end{itemize}
